...
class GeoTiff(object):
        ...
        for i in range(row_num):
            for j in range(col_num):
                ...
                for id in range(len(ins_list)):
                    ...
                    try:
                        save_path = os.path.join(out_dir, '%d_%s_%d.tif' % (begin_id+i*col_num+j, 'buildingfootprint', id))
    ...
    def clip_tif_and_shapefile(self, clip_size, begin_id, shapefile_path, out_dir,datasetname = 'buildingfootprint',ann_path = 'annotations'):
        # import pdb
        # pdb.set_trace()
        self.mask_tif_with_shapefile(shapefile_path)
        self.clip_mask_with_grid(clip_size=clip_size, begin_id=begin_id, out_dir=out_dir + '/'+ann_path)
        pic_id = self.clip_tif_with_grid(clip_size=clip_size, begin_id=begin_id, out_dir=out_dir + '/'+datasetname)
        return pic_id
...
def clip_from_file(clip_size, root, img_path, shp_path,datasetname = 'buildingfootprint',ann_path = 'annotations'):
    ...
    for i in range(n_img):
        ...
        # pic_num = tif.clip_tif_and_shapefile(clip_size, pic_id, root + '/' + shp_path + '/' + img_id + '/' + img_id + '.shp', root + '/dataset',datasetname = datasetname,ann_path = 'annotations')
        pic_num = tif.clip_tif_and_shapefile(clip_size, pic_id, root + '/' + shp_path + '/' + img_id + '/' + img_id + '.shp', root,datasetname = datasetname,ann_path = 'annotations')
        pic_id += pic_num
