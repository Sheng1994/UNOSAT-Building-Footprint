...
def copy_data(input_path, id, num, mark = 'train',datasetname ='buildingfootprint',ann_path = 'annotations'):
    if num != 0:
        list = os.listdir(input_path + '/' + datasetname)
        ann_list = os.listdir(input_path + '/' + ann_path)
        if not os.path.isdir(input_path + '/' + mark + '/' + datasetname):
            os.makedirs(input_path + '/' + mark + '/' + datasetname)
        if not os.path.isdir(input_path + '/' + mark + '/' + ann_path):
            os.makedirs(input_path + '/' + mark + '/' + ann_path)

        for i in range(num):
            shutil.copyfile(input_path + '/' + datasetname + '/' + list[id[i]], input_path + '/' + mark + '/' + datasetname
                        + '/' + list[id[i]])
            print('From src: ' + datasetname + '/' + list[id[i]] + ' =>dst:' + '/' + mark + '/' + datasetname
                  + '/' + list[id[i]])
            annotation_files = filter_for_annotations(input_path, ann_list, list[id[i]])
            for j in range(len(annotation_files)):
                # pdb.set_trace()
                shutil.copyfile(input_path + '/' + ann_path + '/' + os.path.basename(annotation_files[j]),
...
def slice(input_path, train=0.8, eval=0.2, test=0.0, datasetname = 'buildingfootprint',ann_path = 'annotations'):
    """
    ...
    copy_data(input_path, train_id, n_train, 'train',datasetname =datasetname,ann_path = 'annotations')
    copy_data(input_path, eval_id, n_eval, 'eval',datasetname =datasetname,ann_path = 'annotations')
    copy_data(input_path, test_id, n_test, 'test',datasetname =datasetname,ann_path = 'annotations')
