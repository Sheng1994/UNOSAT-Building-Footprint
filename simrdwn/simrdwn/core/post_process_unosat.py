    ...
    if slice_sizes[0] > 0:
        ...
        for index, row in df.iterrows():
            ...
            for c in row['Seg']:
                ...
                c_c = [int(round(float(c[0]))) + left - pad, int(round(float(c[1]))) + upper - pad]
                seg_coords.append(c_c)
            seg.append(seg_coords)
...
###############################################################################
def post_process_unosat_test_create_df(unosat_test_classes_files, log_file,
                                     testims_dir_tot='',
                                     slice_sizes=[416],
                                     slice_sep='|',
                                     edge_buffer_test=0,
                                     ...
                                     verbose=False):
    """
    Create dataframe from unosat output text files.
    ...
    for i, vfile in enumerate(unosat_test_classes_files):
        ...
        # df = pd.read_csv(vfile, names=['Loc_Tmp', 'Prob',
        #                                         'Xmin', 'Ymin', 'Xmax',
        #                                         'Ymax','Seg'],index_col=None,header=0)


        # import pdb
        # pdb.set_trace()
        # loads(json, precise_float=self.precise_float), dtype=None
        #ValueError: Expected object or value,shoud be lines =True
        df = pd.read_json(vfile,encoding="utf-8",orient='records')
        # set category
        ...
        # Seg_list = []
        # for i in json_data:
        #     if len(json_data[i]['segms']) >= 3:
        #         ######################################################
        #         Loc_Tmp_list.append(json_data[i]['Loc_Tmp'])
        #         Prob_list.append(float(json_data[i]['score']))
        #         Xmin_list.append(json_data[i]['boxes'][0])
        #         Ymin_list.append(json_data[i]['boxes'][1])
        #         Xmax_list.append(json_data[i]['boxes'][2])
        #         Ymax_list.append(json_data[i]['boxes'][3])
        #         Seg_list.append(json_data[i]['segms'])

        # dict = {'Loc_Tmp':Loc_Tmp_list,
        #         'Prob':Prob_list,
        #         'Xmin':Xmin_list,
        #         'Ymin':Ymin_list,
        #         'Xmax':Xmax_list,
