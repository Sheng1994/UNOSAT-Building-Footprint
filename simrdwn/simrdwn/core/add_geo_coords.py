...
def get_row_geo_coords(row, affineObject=[], gdal_geomTransform=[],
    ...
    poly_seg = shapely.geometry.Polygon(row['Seg_Glob'])
    # except ValueError:
    #     continue
    ...
    poly_seg_geo = geomPixel2geomGeo(poly_seg, input_raster=raster_loc,
                                 affineObject=affineObject,
                                 gdal_geomTransform=gdal_geomTransform)
    ...
    return out_arr, poly_geo,poly_seg_geo
...
def add_geo_coords_to_df(df_, inProj_str='epsg:4326', outProj_str='epsg:3857',
    ...
    for idx, row in df_.iterrows():
        ...
        # else, get geo locations
            ...
            # try:
            #     out_arr_row, poly_geo,poly_seg_geo = get_row_geo_coords(
...
#############################################################
            ...
            out_arr.append(out_arr_row)
            if create_geojson:
                out_arr_json.append(poly_seg_geo)
